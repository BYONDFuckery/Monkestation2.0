...
// File: obj/machinery/prize_vendor/pulse_prize.dm

icon_state = "pulse_prize"

name = "Grand Prize Vendor"
desc = "The grand prize!"
dispense_list_override = list(
  code/game/objects/items/crystalrespawn.dm = 1
)
ticket_cost = 250
circuit = /obj/item/circuitboard/machine/prize_vendor/pulse_prize

// Override the vend_prize procedure to handle Academy Ticket dispensing
/proc/vend_prize(mob/user, vended_prize)
{
    if (!vended_prize)
    {
        if (dispense_list_override)
